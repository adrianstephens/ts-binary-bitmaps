...
import {BaseImage, Options, Result, clamp8, greyToRgb, ycbcrToRgb} from './common';
...
const JPEGSegment = {
	...
	_:	bin.Switch('marker', {
		...
		[JPEGMarker.SOF0]:	{sof: JPGblock(SOFData)},
		[JPEGMarker.SOF2]:	{sof: JPGblock(SOFData)},
		[JPEGMarker.SOS]:	SOS,
...
export class JPEG extends BaseImage {
	...
	constructor(public sof: bin.ReadType<typeof SOFData>) {
		super('2d', sof.width, sof.height, {
			Y: {
				width: sof.width * sof.maxH / sof.components[0].h,
				height: sof.height * sof.maxV / sof.components[0].v,
				mips: sof.components[0].blocks ? [sof.components[0].blocks] : [],
			},
			Cb: {
				width: sof.width * sof.maxH / sof.components[1].h,
				height: sof.height * sof.maxV / sof.components[1].v,
				mips: sof.components[1]?.blocks ? [sof.components[1].blocks] : [],
			},
			Cr: {
				width: sof.width * sof.maxH / sof.components[2].h,
				height: sof.height * sof.maxV / sof.components[2].v,
				mips: sof.components[2]?.blocks ? [sof.components[2].blocks] : [],
			}
		});
	}
	async getPixels(options: Options): Promise<Result> {
		...
		for (let my = 0, mi = 0; my < mcusY; my++) {
			...
			for (let mx = 0; mx < mcusX; mx++, mi++) {
				...
				if (sof.components.length === 1) {
					const s = sof.components[0].sampler(mi, maxH, maxV);
					for (let y = y0; y < y1; y++) {
						const sx	= s(y - y0);
					...
					const s0 = sof.components[0].sampler(mi, maxH, maxV);
					const s1 = sof.components[1].sampler(mi, maxH, maxV);
					const s2 = sof.components[2].sampler(mi, maxH, maxV);
					for (let y = y0; y < y1; y++) {
						const sx0	= s0(y - y0);
						const sx1	= s1(y - y0);
						const sx2	= s2(y - y0);
